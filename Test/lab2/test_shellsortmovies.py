...
moviesfile = cf.data_dir + 'Data/SmallMoviesDetailsCleaned.csv'
...
def test_loading_CSV_y_ordenamiento():
    ...
    while not (lt.isEmpty(lst_movies)):
        x = int(lt.removeLast(lst_movies)['goodreads_movie_id'])
        if not (lt.isEmpty(lst_movies)):
            y = int(lt.lastElement(lst_movies)['goodreads_movie_id'])
        else:
            break
        assert x > y
